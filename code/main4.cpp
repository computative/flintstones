...
int main(int, char *[])
{
    // jeg forventer dette skulle gi identiske integraler siden <pq|v|rs> = <rq|v|qs>, men det er ikke riktig. Hvorfor?
    int n1 = 0;
    int ml1= 0;
    int n2 = 0;
    int ml2= -1;
    ...
    int n4 = 0;
    int ml4= 0;
    cout << Coulomb_HO(hw, n1, ml1, n2, ml2, n3, ml3, n4, ml4) << endl;
    n1 = 0;
    ml1= -1;
    n2 = 0;
    ml2= -1;
    ...
    invA(k,alpha,gamma);
    //invA(l,beta,delta);
    cout << alpha << beta << gamma << delta << endl << endl;
    return 0;
    */
    int F = 6; // fermi level
    // truncation limit
    ...
    for (int k = 1; k <= N*N; k++) {
        for (int l = 1; l <= N*N; l++) {
            ...
            if (k == 3 and l == 3) {
                ...
                cout << n1<<ml1<< n2<< ml2<< n3<< ml3<< n4<< ml4 << endl;
                cout << Coulomb_HO(hw, n1, ml1, n2, ml2, n3, ml3, n4, ml4) << endl;
                //return 0;
            }
                ...
                for (int gamma = 0; gamma < N; gamma++) {
                    for (int delta = 0; delta < N; delta ++){
                        ...
                        */
                        ...
                        cout << alpha << beta << gamma << delta << " " << A(alpha+1,beta+1) << A(gamma+1,delta+1) << " " << nninteraction(A(alpha+1,beta+1) , A(gamma+1,delta+1)) << " " << Coulomb_HO(hw, n1, ml1, n2, ml2, n3, ml3, n4, ml4) << endl;
                        cout << n1<<ml1<< n2<< ml2<< n3<< ml3<< n4<< ml4 << endl;
                        if (nninteraction(A(alpha+1,beta+1) , A(gamma+1,delta+1)) != Coulomb_HO(hw, n1, ml1, n2, ml2, n3, ml3, n4, ml4))
                        return 0;
                        HFmatrix(alpha,beta) += rho(gamma,delta)*(                                                                  // (alpha, gamma, delta, beta)
                                    kronecker(sigma(alpha+1), sigma(beta+1))*kronecker(sigma(gamma+1), sigma(delta+1))*nninteraction(A(alpha+1,beta+1) , A(gamma+1,delta+1))
                                  - kronecker(sigma(alpha+1), sigma(delta+1))*kronecker(sigma(gamma+1), sigma(beta+1))*nninteraction(A(alpha+1,delta+1), A(gamma+1, beta+1))
                                    );
                    }
                }
            }
